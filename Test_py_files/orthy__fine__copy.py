...
class ImageOverlayApp:
    ...
    def start_full_control_hotkeys(self):
        """
        Starts listening for the full control keyboard shortcuts using Tab as the modifier key.
        """
        # Define the actions for each key
        key_actions = {
            'w': self.perform_ghost_click,
            'q': self.perform_ghost_click,
            'a': self.perform_ghost_click,
            's': self.perform_ghost_click,
            'x': self.perform_ghost_click,
            'z': self.perform_ghost_click,
        }

        # Map keys to their corresponding action names
        key_to_action = {
            's': 'DistalTip',
            'q': 'NegativeTorque',
            'w': 'PositiveTorque',
        ...
        def on_press(key):
            try:
                ...
                elif self.tab_pressed:
                    if hasattr(key, 'char') and key.char in key_actions:
                        action_name = key_to_action.get(key.char)
                        if action_name:
                            logging.info(f"Shortcut activated: {action_name}")
                            key_actions[key.char](action_name)
            except AttributeError:
                # Special keys (like function keys) can be handled here if needed
                pass

        def on_release(key):
            if key == Key.tab:
                self.tab_pressed = False
                logging.debug("Tab key released.")

        # Start the keyboard listener for full control mode
        ...
        logging.info("Full Control Hotkeys listener started with Tab as modifier.")
